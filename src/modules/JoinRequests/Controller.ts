...
class JoinRequestController {
  ...
  public getOrgJoinRequests = async (
    req: IRequest,
    res: IResponse,
    next: NextFunction
  ): Promise<any> => {
    ...
    try {
      console.log(req.currentOrganization, "req.currentOrganization");
  ...
  public getJoinRequests = async (
    ...
    try {
      ...
      if (user.role === "admin") {
        ...
        const joinRequests = await this.joinReqSvc.getOrgJoinRequests(
